...
{
	[Header("Sounds")]
	[SerializeField] private SoundEvent soundCharacterStep;
	[SerializeField] private SoundEvent soundCharacterStepBig;
	...
	private void Start()
	{
		...
		legLenth = Vector3.Distance(transform.position, footTarget.position);
	...
	{
		footPosition = Vector3.Lerp(footPosition, restPosition + data.playerVel.rb.velocity * 0.08f, Time.deltaTime * 15f);
	...
	private void DoStep()
	{
		if (footDown)
		{
			footDownTime += Time.deltaTime * stepSpeed / scale;
			if (!(footDownTime > 1f) || !otherLeg.footDown || !(Mathf.Abs(data.playerVel.rb.velocity.x) > 1f))
			{
				return;
			}
			StartStep();
			footDownTime = 0f;
			footDown = false;
			if (!data.dead && data.isPlaying && data.isGrounded)
			{
				soundParameterIntensity.Intensity = Mathf.Abs(data.playerVel.rb.velocity.x);
		...
		{
			stepTime += Time.deltaTime * stepSpeed / scale;
			if (stepTime > 1f)
			{
				EndStep();
				stepTime = 0f;
	...
	private void DoRayCast()
	{
		...
		Vector2 origin = data.transform.position + transform.right * transform.localPosition.x;
	...
	private void SetValuesFixed()
	{
		...
		{
			deltaPos = Vector3.Lerp(deltaPos, (Vector2)moveDeltaTransform.position - lastPos, Time.deltaTime * 15f);
